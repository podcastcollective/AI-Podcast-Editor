...
- FALSE STARTS: Only cut when a speaker clearly abandons a sentence and restarts it. You must be very confident the restart is cleaner. If in doubt, leave both. IMPORTANT: When a false start leads into the clean version, make ONE continuous cut from the start of the abandoned text to the start of the final clean version. Do NOT split this into separate cuts — separate cuts leave audible fragments between them. For example, if a speaker says "It's certainly disruptive. It's certainly very disruptive", make a single cut from the start of the first "It's" to the start of the final "It's certainly very disruptive".
- RESTATED THOUGHTS: Only cut when a speaker says something then IMMEDIATELY rephrases the exact same idea and BOTH versions are complete sentences. The second version must fully replace the first with zero loss of meaning. This should be very rare. If the two versions add different nuance or context, keep both. Same rule: make ONE continuous cut covering all the abandoned versions, ending right at the start of the kept version.
