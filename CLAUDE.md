...
**Flow:** Frontend uploads audio → Backend sends to AssemblyAI → Backend polls for transcript → Backend sends compact prompt to Claude → Returns edit decisions → Backend applies cuts with ffmpeg → Backend sends to Adobe Enhance Speech API → Backend finalizes (mastering chain + MP3) → Backend reviews quality (ebur128 + silencedetect) → Returns edited MP3 with quality metrics.
...
- `POST /api/edit-audio` — Apply cuts to audio, returns `job_id`. Requires `ADOBE_ENHANCE_TOKEN`.
- `GET /api/edit-audio-status/<job_id>` — Poll edit status (`cutting` → `enhancing` → `finalizing` → `reviewing` → `completed`)
- `GET /api/edit-audio-download/<job_id>` — Download final MP3 (when `completed`)

## Key Technical Details

- **Single-phase editing:** Cuts → Adobe Enhance → mastering chain (adeclick + alimiter + loudnorm) → quality review (ebur128 + silencedetect) → MP3 export. All automatic, requires `ADOBE_ENHANCE_TOKEN`.
...
- `ADOBE_ENHANCE_TOKEN` — Bearer token from Adobe session. Required for audio editing — pipeline fails without it.
