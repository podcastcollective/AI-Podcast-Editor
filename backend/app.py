...
STRUCTURAL RULES:
- PRE-RECORDING CHAT: Look for recording logistics in the first 60 seconds — "okay recording now", "we are now recording", "are we recording?", "let me hit record", mic checks, countdown cues, etc. These MUST be removed — listeners should never hear them. Cut from the start of the audio up to (but not including) the first word of actual episode content. The episode opener (e.g. "welcome to...", "hey everyone", "hello and welcome", "so today we're...") is REAL CONTENT — it must NEVER be cut. Find that opener, then set end_ms to at least 100ms BEFORE its start_ms to guarantee it is fully preserved with no clipping. When in doubt about whether something is pre-chat or the real opener, keep it — it is far worse to cut the opener than to leave in a second of pre-chat.
